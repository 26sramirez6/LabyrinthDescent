...
AUserCharacter::AUserCharacter() {
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh>
		base_mesh_asset(TEXT("StaticMesh'/Engine/BasicShapes/Cube.Cube'"));
...
void AUserCharacter::tickContinueOnPath(const float _delta_time) {
	...
	while (true) {
		if (LIKELY(m_current_waypoint < _path_size-1)) {
			...
			UE_LOG(LogTemp, Log, TEXT("path size: %d"), _path_size);
	...
	if (UNLIKELY(m_current_waypoint == _path_size-1 &&
	...
	DrawDebugPoint(GetWorld(), m_current_waypoint_vec, 10.f, FColor::Red, false, 1.f, 0);
